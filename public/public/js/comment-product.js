$(document).ready(function(){
  ...
  $('.comment-list').on('click','.js-show-reply',function(){
    ...
    item += '<textarea placeholder="Messege" required class="common-textarea js-reply-content"></textarea>';
    item += '<span class="mess-error reply-error"></span>';
    item += '<a href="javascript:void(0)" data-comment-id='+ commentId +' class="view-btn color-2 btn-comment js-add-reply"><span>Submit Now</span></a>';
    item += '</div>';
      ...
      $.ajax({
        ...
        success: function(data){
          if(data.success){
            ...
            reply.parent().find('.reply-error').text(data.message);
          }
        }
      });
    }
